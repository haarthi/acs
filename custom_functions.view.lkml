  ...
  sql_preamble:
    ...
    ${EXTENDED}
     -- take a dimension, number pair and aggregate as a sum
      ...
      CREATE TEMP FUNCTION pair_with_order(key STRING, value FLOAT64, order_value FLOAT64)
      RETURNS STRUCT<key STRING, value FLOAT64> AS ((
        SELECT STRUCT( CONCAT(CAST(order_value AS STRING), '|', key) as key, value)
      ));

      -- take pairs them sum and convert to a json blob
      CREATE TEMP FUNCTION pairs_sum_graph_ordered(a ARRAY<STRUCT<key STRING, value FLOAT64>>)
      RETURNS STRING AS ((
        SELECT
           CONCAT('chl=',STRING_AGG(key,'|'),'&chd=t:',STRING_AGG(FORMAT("%0.0f",value),','))
        FROM (
          ...
          FROM (
            SELECT
              REGEXP_EXTRACT(a.key, r'\|(.*)$') as key
              , value
              , SAFE_CAST(REGEXP_EXTRACT(a.key, r'(^[^\|]+)|' ) as FLOAT64) as order_value
            FROM UNNEST(pairs_convert_percentage(pairs_sum(a),'total')) a
          )
